...
var enva=document.domain.split(".")[0];
var envb=document.domain.split(".")[2];
//var s_account=(enva == "dev5" || enva == "uat" || enva == "wbportal5") ? "wbroswbcomdevelopment" : "wbroswbcom"
try{
    var pathnameHasAdminOrEdit = (/(^\/?admin|\/edit)/ig).test(document.location.pathname);
    var s_account=(enva == "origin-staging" || enva == "babbitt" || enva == "local" || enva == "staging" || enva == "dev" || envb == "acquia-sites") ? "wbroswbcomdevelopment" : "wbroswbcom"
}catch(e){
    console.warn("Site Catalyst Account toggle Error : ", e );
    s_account = "wbroswbcom";
}
if( s_account != "wbroswbcom" ){
    console.log("Site Catalyst Account  : ", s_account );
}
var s=s_gi(s_account)

//eidr here set which s prop to use
var trackEidr = '';
try{
    if( window['Drupal'] && Drupal.settings && Drupal.settings.trackEidr ){
        trackEidr = Drupal.settings.trackEidr;
       // console.log('EIDR  ', Drupal.settings.trackEidr);
        //set some sprop
    }
}catch(e){
    console.log('Site Catalyst Eidr Tracking Error ::', e);
}
...
s.linkInternalFilters="javascript:,tyrashow.warnerbros.com,extratv.warnerbros.com,peoplescourt.warnerbros.com,judgemathistv.warnerbros.com,ellen.warnerbros.com,www.momlogic.com,www.wbmktg.com,www.wb.com,www2.warnerbros.com,www.warnerbros.com,localhost,#"
...
s.useCommerce=true
s.varUsed="eVar6"
s.eventList="event3,event4,event5" //Abandon,Success,Error
/* Plugin Config */
s.usePlugins=true
function s_doPlugins(s) {
	...
	/* Autocaptures video ID of video playing from URL*/
	s.prop8=s.getQueryParam('play','mediaUrl');
	s.prop8=s.getValOnce(s.prop8,"s_p8",0);
	...
	s.prop14=s.eVar14=s.getTimeParting('h','-8','2008'); // Set hour
	s.prop15=s.eVar15=s.getTimeParting('d','-8','2008'); // Set day
	s.prop16=s.eVar16=s.getTimeParting('w','-8','2008'); // Set Weekend / Weekday
    /* Set the Eidr for the page if it exists */
    s.prop21= s.eVar21 = trackEidr;
...
if(!trackLinks) { var trackLinks = new Object(); };
...
trackLinks = {
    ...
    init: function() {
        ...
        for (var i=anchors.length-1; i>=0; i--) {
            ...
            trackLinks.track(anchors[i], queryString);
        };
    },

    initWithinParent: function(parentId) {
    ...
    track: function(anchor, queryString) {
    	// Track Menu and Footer Links
        if (anchor.className != null && anchor.className != "") {
            trackLinks.addEvent(anchor, 'click', (function(link, type, adid) {
                    return function() {
                        try {
                        	var isPromo = (anchor.className.indexOf('PromoPod') == 0);
                        	var clickType = "";
                        	var linkTrackEvents = "";
                        	var link_elem = (typeof jQuery == 'undefined') ? this : jQuery(this);

                        	if (link.indexOf('wbshop.com') != -1 && link.indexOf('adid=wbp') != -1) {
                        	    var shop_s_account='wbroswhvb2c';
                                var shop_s = s_gi(shop_s_account);
                                shop_s.trackingServer = 'warnerbros.112.2o7.net';
                            	shop_s.linkTrackVars = 'prop18,prop20,eVar34';
                                shop_s.linkTrackEvents = isPromo ? 'Tout' : 'WBShop Click';
                                shop_s.prop18 = type;
                                shop_s.prop20 = type;
                            	shop_s.eVar34 = adid;
                            	clickType = isPromo ? 'Promo Tout Click' : 'WBShop Click';
                            	shop_s.tl(this, 'o', clickType);
                        	}

                        	if (typeof link_elem.getAttribute == 'function' && link_elem.getAttribute("data-omniture").length) {
                        	    clickType = link_elem.getAttribute("data-omniture");
                        	    linkTrackEvents = clickType;
                        	} else {
                        	    clickType = isPromo ? 'Promo Tout Click' : type.split('-')[0] + ' Click';
                        	    linkTrackEvents = isPromo ? 'Tout' : type.split('-')[0];
                        	}

                        	var s_account = (enva == "localhost" || enva == "dev5" || enva == "uat") ? "wbroswbcomdevelopment" : "wbroswbcom";
                        	var s = s_gi(s_account);
                        	s.trackingServer = 'warnerbros.112.2o7.net';
                            s.linkTrackVars = 'prop18,prop20,eVar34';
                ...
                })(anchor.href, anchor.className, queryString['adid'])
            );
        }
    },
...
// auto-init on load
//trackLinks.addEvent(window, 'load', trackLinks.init);
