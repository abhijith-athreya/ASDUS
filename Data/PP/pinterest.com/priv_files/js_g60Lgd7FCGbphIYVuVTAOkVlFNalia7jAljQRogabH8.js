...
(function navigationScript($, Drupal, _) {
  ...
  Drupal.behaviors.navigationNavbar = {
    attach: function (context) {
      ...
      if ($navbar.length > 0) {
        ...
        if ($header.attr('data-has-bg') === 'true' && $('.site__utility').find('#block-tabs, .message').length === 0) {
          $navbar
      ...
      // Apply the proper background to navbar using scrolling data attribute.
      $(window, context).on('scroll', _.debounce(function () {
  ...
  Drupal.behaviors.navigationSubmenu = {
    attach: function (context) {
      ...
      $('.menu--main .menu__item a', context).hoverIntent(function () {
    ...
    attach: function (context) {
      ...
      $toggle.click(function () {
        $(this)
          ...
          .closest('body').toggleClass('u-noscroll');
    ...
    attach: function (context) {
      ...
      function setHeight() {
        if (siteHeight < viewportHeight) {
          $siteContent
          .css('height', siteContentHeight + (viewportHeight - siteHeight))
      ...
      $(window).resize(_.debounce(function () {
        // Re-evaluate some variables.
        siteContentHeight = parseInt($siteContent.height());
        siteHeight = parseInt($('.site', context).height());
...
(function alertsScript($, Drupal, _, drupalSettings) {
  ...
  Drupal.behaviors.alertsSlide = {
    attach: function (context) {
      ...
      var currentBottom = parseInt($alerts.css('bottom'), 10);
      var alertsHeight = $alerts.height();
      var alertsOffset = (alertsHeight - currentBottom) * -1;
      $alerts.css({
        bottom: alertsOffset,
        display: 'block',
        visibility: 'visible'
      ...
      var cookie = getAlertsTrackerCookie($alerts);
      if (!cookie.value) {
        $window.on('scroll', _.debounce(function () {
          ...
          if (scrollPosition >= windowHeight) {
            ...
            $alerts.addClass('alerts--anim').css('bottom', 0);
          }
          else {
            $alerts.css('bottom', alertsOffset);
          }
        }, 15));
      }

      // Allow the alerts to be dismissed.
      $('.js-alerts__close-button').click(function () {
        $alerts
        .css('bottom', alertsOffset)
        .delay(700).hide(0);

        setAlertsTrackerCookie($alerts);
      });
    }
  };
...
(function statsScript($, Drupal, CountUp) {
  ...
  Drupal.behaviors.statNumbers = {
    attach: function (context) {
      ...
      $('.stats--numbers .stat', context).each(function (index) {
        var $stat = $(this);
        ...
        $(statValue).contents().each(function () {
          ...
          if (this.nodeType === 3 && !!$(this).text().trim().match(/^\w+$/)) {
            validStatValue = true;
        ...
        if (validStatValue) {
          ...
          var statCountUp = new CountUp(statValue, statStart, statEnd, 0, duration, countUpOptions);
