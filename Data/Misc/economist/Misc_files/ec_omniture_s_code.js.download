...
function s_doPlugins(s) {
  ...
  var dateString = dateNow.getDate() + ' ' + dateNow.toLocaleString('en-us', { month: "long" }).toLowerCase() + ' ' + dateNow.getFullYear();
  // Set prop10 as follows: datestring | day_of_week | weekend or weekday.
  s.prop10 = dateString + '|' + s.getTimeParting('d', '-5').toLowerCase() + '|' + s.getTimeParting('w', '-5').toLowerCase();
  ...
  // "renewals" and "others".
  var campaignStackValue = '';
  var campaignRenewal = false;
  if (s.campaign) {
    var campaignValue = s.campaign.split('/');
    var attribution = (campaignValue.indexOf('paid') !== -1 || (typeof campaignValue[0] !== 'undefined' && campaignValue[0] === 'd')) ? 'p' : // Paid.
    ...
    switch (attribution) {
      case 'p':
        ...
        s.prop48 = s.eVar48 = s.crossVisitParticipation(campaignStackValue, 's_econcpm', '1000' , '1000', '>', '');
        break;
      case 'o':
      case 'e':
        truncateCookie('s_econcpm1');
        s.prop49 = s.eVar49 = s.crossVisitParticipation(campaignStackValue, 's_econcpm1', '1000' , '1000', '>', '');
        break;
      case 'r':
  ...
  if (campaignStackValue === "other ref" || campaignRenewal === true) {
    ...
    s.prop50 = s.eVar50 = s.crossVisitParticipation(campaignStackValue, 's_econcpm2', '1000' , '1000', '>', '');
  }

  // Ensure these values are only being sent once per session.
  s.campaign = s.getValOnce(s.campaign, "cname", 0);
  var oncePerSession = ["s.prop48", "s.prop49", "s.prop50", "s.prop65", "s.eVar23", "s.eVar48", "s.eVar49", "s.eVar50", "s.eVar55", "s.eVar56", "s.eVar57", "s.eVar58", "s.eVar59", "s.eVar60", "s.eVar65"];
  oncePerSession.forEach(function(a, b) {
    if (typeof s[a.substring(2)] !== 'undefined') {
      s[a.substring(2)] = s.getValOnce(s[a.substring(2)], a.toLowerCase().replace('.', '_'), 0);
    }
  });
  ...
  var user_sub_cookie = s_getCookie('ec_omniture_user_sub');
  if (user_sub_cookie === false) {
    s.prop13 = s.eVar13 = 'anonymous|none';
  }
  else {
    s.prop13 = s.eVar13 = user_sub_cookie; // Set up the default value.
    ...
    if (ec_omniture_user_sub_info) {
      // Prop13 gets the first bit, which can be customer type
      // with or without a | delimited list of entitlements or just the
      // products. This will adjust accordingly.
      var cust_type_or_both = ec_omniture_user_sub_info[0];
      // If there is a pipe, then it is customer type|products and goes into prop13.
      if ( cust_type_or_both.indexOf("|") > -1 ) {
        s.prop13 = s.eVar13 = cust_type_or_both;
      } else {
      // If there is no pipe, then it is customer type alone.
        s.prop13 = s.eVar13 = cust_type_or_both + "|none";
      }
      ...
      // Populate prop53 with the remaining months on the user's subscription.
      var subsRemaining = '';
  ...
  if (s.prop4 === "article" && s.prop13.indexOf('subscriber') === -1 && typeof Drupal === 'object') {
    ...
    if (nidlist !== false) {
      ...
      for (var i = 0; i < nidlist.length; i++) {
        if (nidlist[i].indexOf(Drupal.settings.nid) !== -1) {
          article_count = 0;
        }
      }
  ...
  /*
  First of all we need to get the query parameter from the URL using pure JS (not the Adobe plugin)
  Step 1 - Create calls to ecoquery function that extract the value of a requested query parameter
  Step 2 - for the hidden facebook referrers after the call to the ecoquery function we need to add a criteria to only check that the value contains /fb/
  Step 3 - if step 2 criteria is true then add this:
  s.referrer = s.prop14 = s.eVar14 = "http://www.facebook.com"
  s.prop36 = s.eVar36 = "facebook"

  Step 4: we also want to report on other hidden URLs so repeat step 1 but with the hidref query parameter
  Step 5: If Step 4 is true then add this:
  s.referrer = s.prop14 = s.eVar14 = "http://www." + hidref + ".com"
  */  

  function ecoquery(name, url) {
    ...
    // Calls to ecoquery.
  var fsrc = ecoquery('fsrc');//For facebook social.
  ...
  if (!s.referrer) {
    if (fsrc.indexOf('/fb/') != -1 || fsrc.indexOf('facebook') != -1) {
      s.referrer = "http://www.facebook.com";
      hidrefonce = s.getValOnce(s.referrer, "hidenref", 0);
      if (hidrefonce) {
        s.prop14 = s.eVar14 = s.referrer; 
        s.prop36 = s.eVar36 = "facebook";
      }
    }
    if (fsrc.indexOf('FacebookInstant') != -1) {
      s.referrer = "http://www.instantarticles.fb.com";
      hidrefonce = s.getValOnce(s.referrer, "hidenref", 0);
      if (hidrefonce) {
        s.prop14 = s.eVar14 = s.referrer; 
        s.prop36 = s.eVar36 = "facebookinstant"; 
      }  
    }

    if (!fsrc) {
      var hidref = ecoquery('hidref'); //For all know hidden referrers.
      if (!hidref) { 
        hidref = ""; 
        s.referrer = s.prop14 = s.eVar14 = ""; 
      }
      else { 
        s.referrer = "http://www." + hidref;
        hidrefonce = s.getValOnce(s.referrer, "hidenref", 0); 
        s.prop14 = s.eVar14 = s.referrer; 
      }
    }
...
function isEmpty(v) {
  return !v || !v.hasOwnProperty('length') || v.length === 0; 
}
...
/**
 * Copy props to eVars, except for those blacklisted.
 */
function seteVars(s) {
  var propBlackList = [9, 13, 15, 16, 23, 28, 30, 39];
  for (var i = 1; i <= 42; i++) {
    var propStr = "prop" + i;
    var evarStr = "eVar" + i;
    if (propBlackList.indexOf(i) === -1 && typeof s[propStr] !== 'undefined') {
      s[evarStr] = s[propStr];
    }
  }
}
...
/**
 * Function to check if user has adblocker enabled.
 */
window.onload = function () { 
  full_page_ads = (document.getElementById('gpt_leaderboard_ad') !== null && document.getElementById('gpt_leaderboard_ad').clientHeight == 0 && document.getElementById('gpt_top_mpu_ad') !== null && document.getElementById('gpt_top_mpu_ad').clientHeight == 0 && document.getElementById('block-ec_ads-button_ad_1') !== null && document.getElementById('block-ec_ads-button_ad_1').clientHeight == 0) ? true : false;
  paywall_page_ads = (document.getElementById('gpt_leaderboard_ad') !== null && document.getElementById('gpt_leaderboard_ad').clientHeight == 0 && document.getElementById('ec-regov-iframe') !== null) ? true : false;
  s.prop60 = (full_page_ads || paywall_page_ads) ? 'adblocker exists' : '';
}
...
// Track the testing information - users visiting the revamp and old website
var user_rvjourney = s_getCookie('rvjourney') !== false;
if (user_rvjourney !== false && typeof Drupal.settings.omniture.drupal_version === 'string') {
  s.contextData.revampid = "drupal/" + Drupal.settings.omniture.drupal_version + "/" + user_rvjourney;
}
...
// Subsection to omniture
if (typeof Drupal.settings.omniture.drupal_subsection === 'string') {
  s.contextData.subsection = "drupal/" + Drupal.settings.omniture.drupal_subsection + "/" + user_rvjourney;
}
